...
import com.femuniz.totenninemed.core.interfaces.ITotemApi;
...
public class TotemService {
    private ITotemApi _totemApi;

    public TotemService(Context context){
        Retrofit retrofit = RetrofitClient.GetClient(UrlApiNineMed.Totem, context);
        _totemApi = retrofit.create(ITotemApi.class);
    }

    public void ListaTotensClinica(int idClinica, final IApiCallback<List<Toten>> callback) {
    ...
    public void AdicionaSenhaToten(SenhaToten senhaToten, final IApiCallback<RetornoGenerico<SenhaToten>> callback){
